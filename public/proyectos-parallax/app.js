...
document.addEventListener('DOMContentLoaded', function(){
	...
	// Normalizar imagenes y srcset que contienen 'media/' pero fueron exportadas como "Planos.png/media/..."
	document.querySelectorAll('img').forEach(img => {
		const s = img.getAttribute('src') || '';
		if (!s) return;
		if (/^\/|^https?:\/\//i.test(s)) return; // ya es absoluta
	...
	document.querySelectorAll('[srcset]').forEach(el => {
		...
		if (!v) return;
		const fixed = v.split(',').map(part => {
			let url = part.trim().split(/\s+/)[0];
			if (!/^\/|^https?:\/\//i.test(url) && url.includes('media/')) {
				const idx = url.indexOf('media/');
				url = '/' + url.slice(idx);
			}
			return part.replace(/^[^ ]+/, url);
		}).join(', ');
		el.setAttribute('srcset', fixed);
	});
	console.log('app.js: normalizados src/media si hacía falta');
});
