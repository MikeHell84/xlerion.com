...
<script>
  (function(){
    try{
      ...
      var days = Array.from(new Set(raw.map(function(r){ return r.day; }))).sort();
      if (days.length === 0) {
        var d = new Date(); var labels = [];
        for (var i=6;i>=0;i--){ var dd = new Date(); dd.setDate(d.getDate()-i); labels.push(dd.toISOString().slice(0,10)); }
        days = labels;
      }
      var slugs = Array.from(new Set(raw.map(function(r){ return r.slug; }))).filter(Boolean);
      var datasets = slugs.map(function(s, idx){
        var data = days.map(function(d){
          var found = raw.find(function(rr){ return rr.slug === s && rr.day === d; });
          return found ? parseInt(found.cnt,10) : 0;
        });
        return { label: s || '(sin slug)', data: data, borderColor: 'hsl(' + ((idx*47)%360) + ',70%,40%)', backgroundColor: 'rgba(0,0,0,0)', fill:false };
      });
      var ctx = document.getElementById('chartPageViews');
      if (ctx && slugs.length>0) new Chart(ctx.getContext('2d'), { type:'line', data:{ labels: days, datasets: datasets }, options:{ responsive:true, plugins:{legend:{position:'bottom'}} } });
